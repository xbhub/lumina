...
layui.define(["layer", "element"], function (exports) {
    ...
    $body.on("click", "*[lay-href]", function () {
        var $this = $(this),
            ...
            _target = $this.attr("lay-target") || 'tab',
            _text = $this.attr("lay-text");
        layui.router();

        Admin.tabsPage.elem = $this;
        if(_target == 'modal') {
            top.layui.admin.openModal(_href, _text || $this.text())
        }else{
    ...
    Admin.openTab = function(href, title){
        ...
        if (title = title || "新标签页") {
            if(!hasOpened){
                $appBody.append(['<div class="lumina-tabsbody-item layui-show">', '<iframe src="' + href + '" onload="$(this).removeClass(\'layui-line-load\')" frameborder="0" class="lumina-iframe layui-line-load"></iframe>', "</div>"].join(""));
    ...
    // 全局ajax监听
    $body.on("click", "*[lay-ajax-get]", function (e) {
        e.preventDefault();

        var $this = $(this),
            CLASS_DISABLE = 'disabled',
            LOAD = '<i class="fa fa-spinner fa-spin ml-1"></i>',
        ...
        $this.addClass(CLASS_DISABLE).append(LOAD)
        Admin.request.get(_href, {}, function(res){
            if(res.msg) {
                layer.msg(res.msg)
            }
            ...
            $this.find('i').remove();
            $this.removeClass(CLASS_DISABLE);
        })
        return false;
    });
