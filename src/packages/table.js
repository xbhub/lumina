...
layui.define(['admin', 'laytpl', 'laypage', 'form', 'dropdown','util'], function (exports) {
    "use strict";
    ...
    var $ = layui.$,
        ...
        TPL_HEADER = function (options) {
            ...
            options = options || {};
            return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table uppercase" ', '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', '<thead>', '{{# layui.each(d.data.cols, function(i1, item1){ }}', '<tr>', '{{# layui.each(item1, function(i2, item2){ }}', '{{# if(item2.fixed && item2.fixed !== "right"){ left = true; } }}', '{{# if(item2.fixed === "right"){ right = true; } }}', function () {
        ...
        TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
            ...
            , '<div class="layui-table-box">', '{{# if(d.data.loading){ }}', '<div class="layui-table-init" style="background-color: #fff;">', '<i class="fa fa-spin fa-spinner"></i>', '</div>', '{{# } }}'
    ...
    Class.prototype.initOpts = function (item) {
        ...
        // 追加查看列
        if (options.autoShow && !autoshowCol) {
            autoshowCol = true
            options.cols[0].push({
                "title": "#",
                "toolbar": "<div><a class='px-1 cursor-pointer' lay-event='show'><i class='fa fa-eye'></i></a></div>",
                "fixed": "right",
                "width": 50,
                "downoff": true
            })
    ...
    Class.prototype.renderToolbar = function () {
        ...
        var leftDefaultTemp = {
            ...
            leftToolElem = ['<div class="layui-inline tool px-3 py-1 mr-2" lay-event="LAYTABLE_REFRESH"><i class="fa fa-refresh"></i></div>'],
        ...
        if (options.action) {
            var actionItems = '';
            var leftActionTemp = [
                '<div class="layui-inline tool ml-2 px-3 py-1 layui-btn-dropdown" data-toggle="dropdown">',
                '<i class="fa fa-ellipsis-v px-2"></i>',
                '<ul class="layui-dropdown-menu">{{ d.actionItems }}</ul>',
                '</div></div>'
            ].join('');
            $.each(options.action, function (i, n) {
                actionItems += '<li><a lay-event="' + n.event + '">' + n.text + '</a></li>'
            })
            elemToolTemp.append(laytpl(leftActionTemp).render({
                'actionItems': actionItems
            }))
            dropdown.render();
        }
        ...
        if (typeof options.defaultToolbar === 'object') {
            layui.each(options.defaultToolbar, function (i, item) {
                ...
                if (thisItem && (item !== 'exports' || options.export.can)) {
                    iconElem.push('<div class="layui-inline tool px-3 py-1" title="' + thisItem.title + '" lay-event="' + thisItem.layEvent + '">' +
        ...
        if (options.canSearch) {
            var searchElem = '<div class="layui-table-tool-search"><form class="layui-form"><input type="text" name="wd" value="' + (options.where.search || '' )+ '"><button lay-submit lay-filter="*" data-table="'+options.id+'" class="fa fa-search"></button></form></div>';

            that.layTool.prepend(searchElem);
            that.layTool.find(".layui-table-tool-temp").css('marginLeft', '265px');

            if (options.where.search) {
                var _wdIpt = that.layTool.find(".layui-table-tool-search input[name=wd]"),
                    _val = _wdIpt.val();
                _wdIpt.val("").focus().val(_val)
                that.layTool.find("form").append('<button class="j_cancel fa fa-close mr-6" lay-submit lay-filter="*" data-table="'+options.id+'" data-type="cancel"></button>')
            }else{
                that.layTool.find("form .j_cancel").remove();
            }
        }
    }
            ...
            util.request.ajax({
                ...
                success: function (res) {
                    ...
                    if (res[response.statusName] != response.statusCode) {
                        ...
                        that.renderData(res, curr, res['meta'] ? res['meta'][response.countName] : res.data.length);
                        options.time = (new Date().getTime() - that.startTime) + ' ms'; //耗时（接口请求+视图渲染）
                    }
                    that.setColsWidth();
                    that.renderDone(res, curr, res[response.countName]);
                },
                error: function (e, m) {
    ...
    Class.prototype.renderDone = function (res, curr, countName) {
        var that = this,
            options = that.config;

        // 高亮sort
        if (options.where.orderBy) {
            var $sortWrap = that.layHeader.find('[data-field=' + options.where.orderBy + ']').find('.layui-table-sort');
            if ($sortWrap.length > 0) {
                $sortWrap.attr('lay-sort', options.where.sortedBy);
            }
        }
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
        ...
        if (options.page) {
            options.page = $.extend({
                ...
                count: count,
                limit: options.limit,
                limits: options.limits || [15, 50, 200, 500],
                groups: 3,
    ...
    Class.prototype.loading = function (hide) {
        ...
        if (options.loading) {
            if (hide) {
                ...
                that.layInit = $(['<div class="layui-table-init">', '<i class="fa fa-spinner fa-spin"></i>', '</div>'].join(''));
    ...
    Class.prototype.events = function () {
                ...
                case 'LAYTABLE_EXPORT': //导出
                    openPanel({
                        list: function () {
                            ...
                            var loadEl = '<i class="fa fa-spinner fa-spin ml-1"></i>',
                                doing = false;
                            ...
                            list.on('click', function () {
                                ...
                                if (type === 'checked') {
                                    var checked = table.checkStatus(options.id);
                                    if (checked.data.length < 1) {
                                        layer.msg('请选择需要下载的数据');
                                        return false;
                                    }
                                    ids = checked.data.map(function (n) {
                                        return n.id
                                    }).join(',')
                                } else if (type === 'all') {
                                    ids = 'all';
                                ...
                                layui.util.request.ajax({
                                    ...
                                    success: function (res) {
                                        ...
                                        alink.href = res.data.file
                                        alink.download = res.data.name
                                        parent.document.body.appendChild(alink);
                                        alink.click();
                                        parent.document.body.removeChild(alink);
        ...
        }).on('click', '.layui-table-fixed-r tr', function () { //单击行
        ...
        var ELEM_GRID = 'layui-table-grid',
            ...
            gridExpand = function (hide) {
                ...
                } else if (!othis.data('downoff') && elemCell.prop('scrollWidth') > elemCell.outerWidth()) {
                    if (elemCell.find('.' + ELEM_GRID_DOWN)[0]) return;
                    othis.append('<div class="' + ELEM_GRID_DOWN + '"><i class="fa fa-down"></i></div>');
                }
            };
        ...
        that.layBody.on('click', '.' + ELEM_GRID_DOWN, function (e) {
            ...
            that.tipsIndex = layer.tips([
                '<div class="layui-table-tips-main" style="margin-top: -' + (elemCell.height() + 16) + 'px;' + function () {
                    ...
                    return '';
                }() + '">', elemCell.html(), '</div>', '<i class="fa layui-table-tips-c fa-close"></i>'
            ].join(''), elemCell[0], {
                tips: [3, ''],
                time: -1,
                anim: -1,
        ...
        that.layBody.on('click', '*[lay-event]', function () {
            ...
            var obj = commonMember.call(this, {
                event: event
            });
            if (options.autoShow && event == 'show') {
                $.ajax({
                    type: 'get',
                ...
                layui.event.call(this, MOD_NAME, 'tool(' + filter + ')', obj);
            }
            that.setThisRowChecked(index);
        });
    ...
    table.setChecked = function (id, index, checked) {
        var config = getThisTableConfig(id); //获取当前实例配置项
        if (!config) return;
        var data = table.cache[id] || [];

        data[index][options.checkName] = checked;
    }
