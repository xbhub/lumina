...
layui.define(['laydate', 'upload', 'wangEditor'], function (exports) {
    "use strict";
    ...
    var FromEvent = {
        img: function () {
            var _imgItem = '<li class="uploader__file" style="background-image: url(UPLOADEDFILE)"><div class="uploader__mask" style="display: none">' +
                ...
                '</div></li>',
                $self = $(this),
                $modelId = $(this).parents(FORM_WRAP),
                $imgbox = $modelId.find(FORM_BOX),
                $iptVal = $modelId.find(FORM_VAL),
                $imgPicker = $modelId.find(".img__picker"),

                upload_limit = $self.data("limit") || 1;
        ...
        init: function () {
            var _render = {
                ide: function (n) {
                    $dom.find(n.elem).each(function (n, i) {
                        var $self = $(this);
                        var editor = new wangEditor($self.attr('id'));
                ...
                img: function () {
                    var $wrap = $dom.find(FORM_WRAP);
                    var _limit = $wrap.find("#j_uploader_box").data("limit");
                    var $iptVal = $wrap.find(FORM_VAL),
                        $imgPicker = $wrap.find(".img__picker");

                    $wrap.on("mouseenter", ".uploader__file", function () {
                        $(this).find(".uploader__mask").show()
                    }).on("mouseleave", ".uploader__file", function () {
                        $(this).find(".uploader__mask").hide()
                    })
                    $wrap.on("click", ".j_delete", function () {
                        if (_limit == 1) {
                ...
                date: function (n) {
                    $dom.find(n.elem).each(function (i, n) {
                        var $self = $(n);
                        laydate.render({
                            elem: '#' + $self.attr('id'),
                            type: 'date',
            ...
            $.each(FORM_RENDER, function (i, n) {
                var _type = n.type
                _render[_type] && _render[n.type].call(this, n)
            })
        }
    }

    // 组件库
    $dom.on("click", FORM_IMG, FromEvent.img)
